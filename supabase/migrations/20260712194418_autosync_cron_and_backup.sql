-- Re-enable automatic sync (imported_videos + youtube_manga_videos) via
-- pg_cron, and install local backup mirrors of both tables.
...
-- ---------- Cron: invoke edge functions on a schedule ----------
-- The functions are publicly invokable (no verify_jwt requirement); pg_net
-- posts to them using the project anon key so both YouTube import paths run
-- automatically without the admin UI.
...
begin
  -- Unschedule any previous runs so re-applying the migration is idempotent.
